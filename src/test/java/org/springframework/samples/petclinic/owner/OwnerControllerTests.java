...
class OwnerControllerTests {
	...
	void setup() {
		...
		Owner george = george();
		// Mock the new search method for various scenarios
		given(this.owners.findByLastNameContainingIgnoreCaseOrderByLastNameAsc(eq("Franklin"), any(Pageable.class)))
			.willReturn(new PageImpl<>(List.of(george)));
		given(this.owners.findByLastNameContainingIgnoreCaseOrderByLastNameAsc(eq("franklin"), any(Pageable.class)))
			.willReturn(new PageImpl<>(List.of(george)));
		given(this.owners.findByLastNameContainingIgnoreCaseOrderByLastNameAsc(eq("frank"), any(Pageable.class)))
			.willReturn(new PageImpl<>(List.of(george)));
	...
	void processFindFormSuccess() throws Exception {
		...
		when(this.owners.findByLastNameContainingIgnoreCaseOrderByLastNameAsc(anyString(), any(Pageable.class)))
			.thenReturn(tasks);
	...
	void processFindFormByLastName() throws Exception {
		...
		// Updated to use the new search method
		when(this.owners.findByLastNameContainingIgnoreCaseOrderByLastNameAsc(eq("Franklin"), any(Pageable.class)))
			.thenReturn(tasks);
	...
	@Test
	void processFindFormByLastNameCaseInsensitive() throws Exception {
		Page<Owner> tasks = new PageImpl<>(List.of(george()));
		when(this.owners.findByLastNameContainingIgnoreCaseOrderByLastNameAsc(eq("franklin"), any(Pageable.class)))
			.thenReturn(tasks);
		mockMvc.perform(get("/owners?page=1").param("lastName", "franklin")) // lowercase
			.andExpect(status().is3xxRedirection())
			.andExpect(view().name("redirect:/owners/" + TEST_OWNER_ID));
	...
	@Test
	void processFindFormByLastNamePartialMatch() throws Exception {
		Page<Owner> tasks = new PageImpl<>(List.of(george()));
		when(this.owners.findByLastNameContainingIgnoreCaseOrderByLastNameAsc(eq("frank"), any(Pageable.class)))
			.thenReturn(tasks);
		mockMvc.perform(get("/owners?page=1").param("lastName", "frank")) // partial match
			.andExpect(status().is3xxRedirection())
			.andExpect(view().name("redirect:/owners/" + TEST_OWNER_ID));
		verify(this.owners).findByLastNameContainingIgnoreCaseOrderByLastNameAsc(eq("frank"), any(Pageable.class));
	}
	...
	void processFindFormWithWhitespaceOnlyLastNameReturnsAllOwners() throws Exception {
		Page<Owner> tasks = new PageImpl<>(List.of(george(), new Owner()));
		// Updated to use the new search method
		when(this.owners.findByLastNameContainingIgnoreCaseOrderByLastNameAsc(eq(""), any(Pageable.class)))
			.thenReturn(tasks);
	...
	void processFindFormNoOwnersFound() throws Exception {
		...
		// Updated to use the new search method
		when(this.owners.findByLastNameContainingIgnoreCaseOrderByLastNameAsc(eq("Unknown Surname"),
				any(Pageable.class)))
			.thenReturn(tasks);

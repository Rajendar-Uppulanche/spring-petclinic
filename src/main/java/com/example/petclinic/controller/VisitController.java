...
class VisitController {
	...
	private final VisitTypeRepository visitTypes;
	private final VeterinarianRepository veterinarians;

	public VisitController(OwnerRepository owners, VisitTypeRepository visitTypes, VeterinarianRepository veterinarians) {
		this.owners = owners;
	...
	public java.util.Collection<VisitType> populateVisitTypes() {
		return visitTypes.findAll();
	}

	@ModelAttribute("veterinarians")
	public java.util.Collection<Veterinarian> populateVeterinarians() {
		return veterinarians.findAll();
	}
	...
	public String initNewVisitForm(@PathVariable int ownerId, @PathVariable int petId,
			Map<String, Object> model) {
	...
	public String processNewVisitForm(@ModelAttribute Owner owner, @PathVariable int petId,
		...
		if (visit.getDate() != null && visit.getDate().isAfter(LocalDate.now())) {
			result.rejectValue("date", "error.visitDateFuture", "Visit date cannot be in the future.");
		}

		if (visit.getVisitType() == null || visit.getVisitType().getId() == null) {
			result.rejectValue("visitType", "error.visitTypeRequired", "Visit type is required.");
		}
	...
	public String initUpdateVisitForm(@PathVariable int ownerId, @PathVariable int petId,
			@PathVariable int visitId, Map<String, Object> model) {
	...
	public String processUpdateVisitForm(@ModelAttribute Owner owner, @PathVariable int petId,
		...
		Visit existingVisit = owner.getPet(petId).getVisit(visitId);
		...
		owner.updateVisit(petId, existingVisit);
		this.owners.save(owner);

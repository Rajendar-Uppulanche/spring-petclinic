...
class VisitController {
	...
	public String processUpdateVisitForm(@Valid Visit visit, BindingResult result, @PathVariable int visitId,
			@PathVariable int ownerId, @PathVariable int petId, RedirectAttributes redirectAttributes) {

		if (result.hasErrors()) {
			return "pets/createOrUpdateVisitForm";
		}

		Visit existingVisit = this.visitService.findById(visitId);
		existingVisit.setDescription(visit.getDescription());
		// Ensure date and type are not modified
		// existingVisit.setDate(visit.getDate()); // This should not be allowed
		// existingVisit.setType(visit.getType()); // This should not be allowed

		this.visitService.save(existingVisit);
		redirectAttributes.addFlashAttribute("message", "Your visit has been updated");
		return "redirect:/owners/{ownerId}/pets/{petId}/visits/{visitId}";
	}
